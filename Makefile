...
help:
	...
	@echo "\ttype [DISABLED] - checks Python typing (uses tox)"
	...
	@echo "\tpy36 [DISABLED] - run tests quickly with the default Python 3.6 (uses tox)"
	@echo "\tpy37 [DISABLED] - run tests quickly with the default Python 3.7 (uses tox)"
	@echo "\tpy38 [DISABLED] - run tests quickly with the default Python 3.8 (uses tox)"
...
dev-env-full = $(dev-env-minimal) -E test -E bdd -E lint -E repl
...
test:
	poetry run pytest --cov=behave4git --cov-report=html --cov-report=term tests
